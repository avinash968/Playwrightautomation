...
async function executeWorkflow() {
  ...
  try {
    ...
    for (const [clientKey, clientData] of Object.entries(CLIENTS)) {
      ...
      const orderFound = await page
        .textContent("body")
        .then(
          (content) =>
            content.includes(clientData.searchEmail) ||
            content.includes(clientData.searchPhone),
        );
      ...
      if (rowLocator >= 0) {
        ...
        await page.click(
          `tr:nth-of-type(${rowLocator + 1}), [role="row"]:nth-of-type(${rowLocator + 1})`,
        );
        await page.waitForNavigation({ waitUntil: "networkidle" });
      }

      // Take order details screenshot
      const detailsScreenshot = path.join(
        clientData.screenshotDir,
        "03-order-details.png",
      );
      ...
      await page.evaluate(() => {
        const notes = Array.from(document.querySelectorAll("h3, h2, div")).find(
          (el) => el.textContent.includes("Notes"),
        );
        notes?.scrollIntoView({ behavior: "smooth" });
      });

      await page.waitForTimeout(1000);
      const notesScreenshot = path.join(
        clientData.screenshotDir,
        "04-notes-section.png",
      );
      await page.screenshot({ path: notesScreenshot });
      console.log(`  📸 Notes section screenshot: ${notesScreenshot}`);

      // Validate active price
      const activePrice = await page.textContent(
        '[class*="price"], [class*="active"], [data-testid*="price"]',
      );
      const activePriceScreenshot = path.join(
        clientData.screenshotDir,
        "05-active-price.png",
      );
    ...
    const totalPass = Object.values(validationResults).reduce(
      (sum, r) => sum + r.passed,
      0,
    );
    const totalFail = Object.values(validationResults).reduce(
      (sum, r) => sum + r.failed,
      0,
    );
    ...
    console.log(
      `📊 Summary: ${totalPass} PASS, ${totalFail} FAIL, Overall: ${overallStatus}`,
    );
    console.log(
      `✅ Google Chat notification would be sent (notify-email-intercept.ps1)`,
    );
    ...
    console.log(
      '📝 Run: git add . && git commit -m "Email Intercept QA Workflow Results"',
    );
  } finally {
    await browser.close();
  }
